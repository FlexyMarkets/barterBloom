import { Button, Card, Container, Divider, Stack, Typography, TextField, InputLabel, OutlinedInput, InputAdornment } from '@mui/material'
import Grid from "@mui/material/Grid2"
import { allSecurityQuestion } from "./setTransactionPasswordFormData"
...
import { useSetTransactionPasswordMutation } from '../../../../../globalState/settings/profileSettingApi'
...
function SetTransactionPasswordForm() {
  ...
  const { register, handleSubmit, setError, reset, watch, setValue, formState: { errors } } = useForm({
          ...
          >
            <Grid container size={12} spacing={3}>
              <Grid item size={{ xs: 12, sm: 6, md: 4 }}>
                <InputLabel sx={{ mb: ".5rem" }}>Security Question</InputLabel>
                <Selector
                  items={allSecurityQuestion}
                  value={watch("securityQuestion")}
                  onChange={(e) => setValue("securityQuestion", e.target.value, { shouldValidate: true })}
                  placeholder={"Select a question --"}
                  shouldBeFullWidth={true}
                />
                {errors.securityQuestion && <Typography color="error">{errors.securityQuestion.message}</Typography>}
              </Grid>
              <Grid item size={{ xs: 12, sm: 6, md: 4 }}>
                <InputLabel sx={{ mb: ".5rem" }}>Answer</InputLabel>
                <TextField
